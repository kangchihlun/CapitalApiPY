...
class Event:
    ...
    #----------------------------------------------------------------------
    def __init__(self,_type_,AnyValue_=[]):
        """Constructor"""
        self.type_ = _type_      # 事件类型
...
class EventEngine(object):
    ...
    def __process(self, event):
        """处理事件"""
        # 检查是否存在对该事件进行监听的处理函数
        #WinForm.MessageBox.Show( '__process event , Length of handler =' + len(self.__handlers[event.type_]) )
    ...
    def register(self, type_, handler): 
        ...
        if handler not in handlerList:
            ...
            # WinForm.MessageBox.Show( 'register handler type = '+ type_ +'::'+ str(handler)) # ok
    ...
    def put(self, event):
        """向事件队列中存入事件"""
        # WinForm.MessageBox.Show( 'put event ' + str(event)) #ok
...
class EventEngine2(object):
    ...
    def __init__(self):
        ...
        # 计时器，用于触发计时器事件
        self.__timer = threading.Thread(target = self.__runTimer)
    ...
    def __runTimer(self):
        ...
        while self.__timerActive:
            # 创建计时器事件
            event = Event(_type_=EVENT_TIMER)
...
def test():
    ...
    def simpletest(event):
        print ('timer event %s' % str(datetime.now()))
